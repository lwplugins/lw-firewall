 ...
 */
import FirewallMark from '../components/FirewallMark';
		...
		<aside className={ `lw-admin-sidebar ${ isOpen ? 'is-open' : '' }` }>
			<div className="lw-admin-sidebar__head">
				<a
					...
					href={ `#${ tabs[ 0 ].id }` }
					aria-label={ __( 'LW Firewall home', 'lw-firewall' ) }
				>
					<FirewallMark />
			...
			<nav
				...
				aria-label={ __( 'LW Firewall sections', 'lw-firewall' ) }
			>
				<ul>
					{ tabs.map( ( tab ) => {
			...
			<div className="lw-admin-sidebar__foot">
				...
				>
					...
					<span className="lw-admin-sidenav__label">
						{ __( 'Documentation', 'lw-firewall' ) }
					</span>
					<Icon icon={ external } size={ 16 } />
					<span className="screen-reader-text">
						{ __( '(opens in a new tab)', 'lw-firewall' ) }
					</span>
				</a>
			</div>
		</aside>
