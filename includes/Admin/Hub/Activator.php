...
final class Activator {
	...
	public function activate( string $slug ): ?WP_Error {
		if ( null === $this->catalog->entry( $slug ) ) {
			return new WP_Error( 'lw_hub_unknown_plugin', __( 'This is not an LW plugin.', 'lw-firewall' ), array( 'status' => 404 ) );
		}

		$file = $this->catalog->detector()->find_file( $slug );

		if ( null === $file ) {
			return new WP_Error( 'lw_hub_not_installed', __( 'This plugin is not installed.', 'lw-firewall' ), array( 'status' => 409 ) );
		}
		...
		if ( ! current_user_can( 'activate_plugin', $file ) ) {
			return new WP_Error( 'lw_hub_forbidden', __( 'You are not allowed to activate this plugin.', 'lw-firewall' ), array( 'status' => 403 ) );
		}
